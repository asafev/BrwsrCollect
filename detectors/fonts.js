...
class FontsDetector {
    ...
    /**
     * Test fonts using offset measurement technique (BrowserLeaks/classic method)
     * This is more reliable than FontFace.load() for detecting system fonts
     * @private
     */
    async _testFonts() {
        const available = [];
        ...
        // Create test span with characters that vary significantly across fonts
        const testSpan = document.createElement('span');
        testSpan.style.fontSize = '72px';
        testSpan.style.fontStyle = 'normal';
        testSpan.style.fontWeight = 'normal';
        testSpan.style.letterSpacing = 'normal';
        testSpan.style.lineHeight = 'normal';
        testSpan.style.textTransform = 'none';
        testSpan.style.textAlign = 'left';
        testSpan.style.textDecoration = 'none';
        testSpan.style.whiteSpace = 'nowrap';
        // Use a test string with varied characters (similar to BrowserLeaks)
        testSpan.textContent = 'mmmmmmmmmmlli';
        testContainer.appendChild(testSpan);
        
        // Baseline fonts to compare against
        const baseFonts = ['monospace', 'sans-serif', 'serif'];
        
        // Measure baseline dimensions for each base font
        ...
        for (const baseFont of baseFonts) {
            ...
            baselineHeights[baseFont] = testSpan.offsetHeight;
        }
        ...
        for (const fontFamily of FONT_LIST) {
            ...
            for (const baseFont of baseFonts) {
                // Set font with fallback to base font
                testSpan.style.fontFamily = `"${fontFamily}", ${baseFont}`;
                
                const width = testSpan.offsetWidth;
                const height = testSpan.offsetHeight;
                
                // If dimensions differ from baseline, font is installed
                if (width !== baselineWidths[baseFont] || height !== baselineHeights[baseFont]) {
                    detected = true;
                    break;
                }
            }
     ...
     * Uses a running hash like BrowserLeaks for compact fingerprinting
     * @private
     */
    async _measureEmoji() {
        try {
            ...
            // Running hash function (same as BrowserLeaks)
            ...
            for (const emoji of EMOJI_CHARS) {
                ...
                // Get text metrics for width/height
                const metrics = ctx.measureText(emoji);
                const width = Math.round(metrics.width);
                
                // Update running hash with dimensions (BrowserLeaks method)
            ...
            // Format hash as 8-char hex string (like BrowserLeaks)
    ...
    _formatMetrics(result) {
        return {
            ...
            fontsEmojiRunningHash: {
                value: result.emoji && result.emoji.runningHash ? result.emoji.runningHash : null,
                description: 'Running hash of emoji dimensions (BrowserLeaks method)',
                risk: 'N/A'
            },
            fontsEmojiHash: {
